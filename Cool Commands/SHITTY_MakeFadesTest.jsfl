...
for(var i = 0; i < startingFrames.length; i++) {
	...
	if(prevCharacter != frameToCharacterMap[startingFrames[i]] && startingFrames[i] != 0) {
		...
		fl.trace(frameToSymbolMap[startingFrames[i]]);
...
var timeDiff = setupTime - startTime;
timeDiff /= 1000;
var seconds = Math.round(timeDiff);

if (timeDiff < 60) {
    fl.trace("Time Elapsed for setup: " + seconds + " seconds.");
}

if (timeDiff > 60) {
    var minutes = Math.floor(timeDiff / 60);
    var seconds = timeDiff - minutes * 60;
    fl.trace("Time Elapsed for setup: " + minutes + " minutes and " + seconds + " seconds");
}
/*for(var i = 0; i < startingFrames.length; i++) {
	fl.trace(startingFrames[i] + ", " + endingFrames[startingFrames[i]] + ", " + frameToCharacterMap[startingFrames[i]]);
}*/

library.newFolder("FADES");
...
for(var i = 0;  i < startingFrames.length; i++) {
	...
	var isRaster = (frameToCharacterMap[startingFrames[i]].indexOf("WRIGHT") != - 1) || (frameToCharacterMap[startingFrames[i]].indexOf("APOLLO") != -1);
	if(prevCharacter != frameToCharacterMap[startingFrames[i]] && startingFrames[i] != 0 && !alreadyFaded[startingFrames[i]]) {
		...
		doc.selection[0].firstFrame = realStartingFrames[startingFrames[i]];
		var isApollo = false;
		if(!library.itemExists("FADES/" + frameToCharacterMap[startingFrames[i]] + " " + poseNames[startingFrames[i]] + " FADE IN")) {
			if((frameToCharacterMap[startingFrames[i]].indexOf("APOLLO") != -1)) {
				an.getDocumentDOM().convertToSymbol('movie clip', frameToCharacterMap[startingFrames[i]] + " " + poseNames[startingFrames[i]] + " FADE IN", 'center');
				library.moveToFolder("FADES");
				isApollo = true;
			} else {
			...
			if(!isRaster) {
				...
				doc.addItem({x:0,y:0}, library.items[library.findItemIndex("FADES/" + frameToCharacterMap[startingFrames[i]] + " " + poseNames[startingFrames[i]] + " FADE IN")]);
			} else {
				if((frameToCharacterMap[startingFrames[i]].indexOf("APOLLO") != -1)) {
					an.getDocumentDOM().convertToSymbol('movie clip', frameToCharacterMap[startingFrames[i]] + " " + poseNames[startingFrames[i]] + " FADE IN CLONE " + i, 'center');
					library.moveToFolder("FADES");
					isApollo = true;
				} else {
					doc.convertToSymbol("movie clip", frameToCharacterMap[startingFrames[i]] + " " + poseNames[startingFrames[i]] + " FADE IN CLONE " + i, "top left");
					library.moveToFolder("FADES");
				}
			}
		}
		...
		timeline.setSelectedFrames(timeline.currentFrame, timeline.currentFrame + 1);
		if(!isApollo) {
			library.items[library.findItemIndex("FADES/" + frameToCharacterMap[startingFrames[i]] + " " + poseNames[startingFrames[i]] + " FADE IN")].timeline.layers[0].frames[0].elements[0].x = x;
			library.items[library.findItemIndex("FADES/" + frameToCharacterMap[startingFrames[i]] + " " + poseNames[startingFrames[i]] + " FADE IN")].timeline.layers[0].frames[0].elements[0].y = y;
		}
	...
	prevCharacter = frameToCharacterMap[startingFrames[i]];
	nextCharacter = frameToCharacterMap[startingFrames[i+1]];
	if(nextCharacter != frameToCharacterMap[startingFrames[i]] && startingFrames[i] != 0 && !alreadyFaded[endingFrames[startingFrames[i]]]) {
		...
		if(!library.itemExists("FADES/" + frameToCharacterMap[endingFrames[startingFrames[i]]] + " " + poseNames[endingFrames[startingFrames[i]]] + " FADE OUT")) {
			if((frameToCharacterMap[endingFrames[startingFrames[i]]].indexOf("APOLLO") != -1)) {
				an.getDocumentDOM().convertToSymbol('movie clip', frameToCharacterMap[endingFrames[startingFrames[i]]] + " " + poseNames[endingFrames[startingFrames[i]]] + " FADE OUT", 'center');
				library.moveToFolder("FADES");
				isApollo = true;
			} else {
			...
			if(!isRaster) {
				document.deleteSelection();
				doc.addItem({x:0,y:0}, library.items[library.findItemIndex("FADES/" + frameToCharacterMap[endingFrames[startingFrames[i]]] + " " + poseNames[endingFrames[startingFrames[i]]] + " FADE OUT")]);
			} else {
				if((frameToCharacterMap[startingFrames[i]].indexOf("APOLLO") != -1)) {
					an.getDocumentDOM().convertToSymbol('movie clip', frameToCharacterMap[endingFrames[startingFrames[i]]] + " " + poseNames[endingFrames[startingFrames[i]]] + " FADE OUT CLONE " + i, 'center');
					library.moveToFolder("FADES");
					isApollo = true;
				} else {
					doc.convertToSymbol("movie clip", frameToCharacterMap[endingFrames[startingFrames[i]]] + " " + poseNames[endingFrames[startingFrames[i]]] + " FADE OUT CLONE " + i, "top left");
					library.moveToFolder("FADES");
				}
			}
		}
		...
		timeline.setSelectedFrames(timeline.currentFrame, timeline.currentFrame + 1);
		if(!isApollo) {
			library.items[library.findItemIndex("FADES/" + frameToCharacterMap[endingFrames[startingFrames[i]]] + " " + poseNames[endingFrames[startingFrames[i]]] + " FADE OUT")].timeline.layers[0].frames[0].elements[0].x = x;
			library.items[library.findItemIndex("FADES/" + frameToCharacterMap[endingFrames[startingFrames[i]]] + " " + poseNames[endingFrames[startingFrames[i]]] + " FADE OUT")].timeline.layers[0].frames[0].elements[0].y = y;
		}
	...
	var loopEndTime = new Date();
	timeDiff = loopEndTime - loopStartTime;
	timeDiff /= 1000;
	var seconds = Math.round(timeDiff);

	if (timeDiff < 60) {
		fl.trace("Time Elapsed for fade pair " + i + " (" + frameToCharacterMap[startingFrames[i]] + "): " + seconds + " seconds.");
	}

	if (timeDiff > 60) {
		var minutes = Math.floor(timeDiff / 60);
		var seconds = timeDiff - minutes * 60;
		fl.trace("Time Elapsed for fade pair " + i + " (" + frameToCharacterMap[startingFrames[i]] + "): " + minutes + " minutes and " + seconds + " seconds");
	}
	
}
var fadeTime = new Date();
timeDiff = fadeTime - setupTime;
timeDiff /= 1000;
var seconds = Math.round(timeDiff);

if (timeDiff < 60) {
	fl.trace("Time Elapsed for all fades: " + seconds + " seconds.");
}

if (timeDiff > 60) {
	var minutes = Math.floor(timeDiff / 60);
	var seconds = timeDiff - minutes * 60;
	fl.trace("Time Elapsed for all fades: "+ minutes + " minutes and " + seconds + " seconds");
}
...
var removeFramesTime = new Date();
timeDiff = removeFramesTime - fadeTime;
timeDiff /= 1000;
var seconds = Math.round(timeDiff);

if (timeDiff < 60) {
	fl.trace("Time Elapsed for all frame removals: " + seconds + " seconds.");
}

if (timeDiff > 60) {
	var minutes = Math.floor(timeDiff / 60);
	var seconds = timeDiff - minutes * 60;
	fl.trace("Time Elapsed for all frames removals: "+ minutes + " minutes and " + seconds + " seconds");
}
/*for(var i = 0; i < startingFrames.length; i++) {
	fl.trace(startingFrames[i] + ", " + endingFrames[startingFrames[i]] + ", " + frameToCharacterMap[startingFrames[i]]);
}
