...
fl.documents[toolboxDocIndex].editScene(CONTENT_SCENE); // Flashes scene is the first one
fl.documents[toolboxDocIndex].getTimeline().setSelectedLayers(fl.documents[toolboxDocIndex].getTimeline().findLayerIndex("CONTENT") * 1); // select content layer
fl.documents[toolboxDocIndex].getTimeline().copyFrames(CONTENT_FRAME_BEGIN, CONTENT_FRAME_END + 1); // flash is from frames 1 to 5
