...
if (guiPanel.dismiss == "accept") {
	...
	//If an INTERJECTION layer exists, do it there regardless of input selection. If not, use input.
	if (fl.getDocumentDOM().getTimeline().findLayerIndex("INTERJECTION") !== undefined) {
		layer = fl.getDocumentDOM().getTimeline().findLayerIndex("INTERJECTION")
	}

	fl.getDocumentDOM().getTimeline().layers[layer].visible = true;
	fl.getDocumentDOM().getTimeline().layers[layer - 1].visible = true;
	...
	//Select start of selected frames
	selectOrMakeKeyframe(layer - 1, selectedFrames[1])
